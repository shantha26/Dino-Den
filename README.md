...
- **Roles** — `admin`, `staff`. Each role only sees the nav tabs it's allowed to use (see `TAB_ROLES` in `frontend/src/App.jsx`), and the backend enforces the same restrictions on writes (see `requireRole` calls in `backend/routes/*.js`). Logging in redirects each role to its own home tab (`ROLE_HOME_TAB` in `frontend/src/context/AuthContext.jsx`).
